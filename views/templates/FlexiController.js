  ...
  try {
    ...
    if (result) {
        ...
        try {
          ...
          const emailResult = await Email.sendEmail(template, emailData,templateemail,ccEmails);
          ...
          if (emailResult.accepted.length > 0) {
            ...
            return res.status(422).json({ status: 422,message:`Failed to send email to ${ccEmails}`});
          }
        } catch (error) {
         return res.status(422).json({ status: 422,message:`Error sending email: ${error.message}`});
